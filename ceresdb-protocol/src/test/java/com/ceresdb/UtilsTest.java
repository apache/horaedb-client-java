...
public class UtilsTest {
    ...
    public void combineOkResultTest() {
        ...
        final Result<WriteOk, Err> ok2 = WriteOk.ok(200, 0, Collections.singletonList("m1")).mapToResult();
    ...
    public void splitDataBySingleRouteTest() {
        final List<Rows> data = TestUtil.newListOfRows("m1", "m2", "m3");
        final Map<String, Route> routes = new HashMap<>();
        final Endpoint ep = Endpoint.of("127.0.0.1", 9001);
        routes.put("m1", Route.of("m1", ep));
        routes.put("m2", Route.of("m2", ep));
        routes.put("m3", Route.of("m3", ep));

        final Map<Endpoint, Collection<Rows>> result = Utils.splitDataByRoute(data, routes);

        Assert.assertEquals(1, result.size());
        Assert.assertEquals(ep, result.keySet().stream().findFirst().orElse(null));
        Assert.assertEquals(3, result.values().stream().findFirst().orElse(Collections.emptyList()).size());
    }
    ...
    public void splitDataByMultiRouteTest() {
        ...
        routes.put("m1", Route.of("m1", ep1));
        routes.put("m2", Route.of("m2", ep1));
        routes.put("m3", Route.of("m3", ep2));

        final Map<Endpoint, Collection<Rows>> result = Utils.splitDataByRoute(data, routes);

        Assert.assertEquals(2, result.size());
        Assert.assertEquals(2, result.get(ep1).size());
        Assert.assertEquals(1, result.get(ep2).size());
    }
}
