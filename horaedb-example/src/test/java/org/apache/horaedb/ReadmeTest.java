...
public class ReadmeTest {
    ...
    public void readmeTest() throws ExecutionException, InterruptedException {
        final HoraeDBOptions opts = HoraeDBOptions.newBuilder("127.0.0.1", 8831,
                        RouteMode.DIRECT) // CeresDB default grpc port 8831
        ...
        String createTableSql = "CREATE TABLE IF NOT EXISTS machine_table(" + "ts TIMESTAMP NOT NULL," + //
                "ts TIMESTAMP NOT NULL," + "city STRING TAG NOT NULL," + "ip STRING TAG NOT NULL," + "cpu DOUBLE NULL," + "mem DOUBLE NULL," + "TIMESTAMP KEY(ts)" + // timestamp column must be specified
                ") ENGINE=Analytic";
        ...
        Point point1 = Point.newPointBuilder("machine_table").setTimestamp(timestamp).addTag("city", "Singapore")
                .addTag("ip", "10.0.0.1").addField("cpu", Value.withDouble(0.23))
                .addField("mem", Value.withDouble(0.55)).build();
        Point point2 = Point.newPointBuilder("machine_table").setTimestamp(timestamp + 1000).addTag("city", "Singapore")
                .addTag("ip", "10.0.0.1").addField("cpu", Value.withDouble(0.25))
                .addField("mem", Value.withDouble(0.56)).build();
        Point point3 = Point.newPointBuilder("machine_table").setTimestamp(timestamp + 1000).addTag("city", "Shanghai")
                .addTag("ip", "10.0.0.2").addField("cpu", Value.withDouble(0.21))
                .addField("mem", Value.withDouble(0.52)).build();
        ...
        final SqlQueryRequest queryRequest = SqlQueryRequest.newBuilder().forTables(
                        "machine_table") // table name is optional. If not provided, SQL parser will parse the `sql` to get the table name and do the routing automaticly
                .sql("select * from machine_table where ts >= %d", timestamp).build();
        ...
        for (int i = 0; i < 1000; i++) {
            final Point streamData = Point.newPointBuilder("machine_table").setTimestamp(t).addTag("city", "Beijing")
                    .addTag("ip", "10.0.0.3").addField("cpu", Value.withDouble(0.42))
                    .addField("mem", Value.withDouble(0.67)).build();
        ...
        final SqlQueryRequest streamQuerySql = SqlQueryRequest.newBuilder()
                .sql("select * from %s where city = '%s' and ts >= %d and ts < %d", "machine_table", "Beijing", start,
                        t).build();
