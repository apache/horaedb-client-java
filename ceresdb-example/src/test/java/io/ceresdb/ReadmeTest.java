...
public class ReadmeTest {
    ...
    public void readmeTest() throws ExecutionException, InterruptedException {
        ...
        Assert.assertEquals(timestamp, rows.get(0).getColumn("ts").value().getTimestamp());
        Assert.assertEquals("Singapore", rows.get(0).getColumn("city").value().getString());
        Assert.assertEquals("10.0.0.1", rows.get(0).getColumn("ip").value().getString());
        Assert.assertEquals(0.23, rows.get(0).getColumn("cpu").value().getDouble(), 0.0000001);
        Assert.assertEquals(0.55, rows.get(0).getColumn("mem").value().getDouble(), 0.0000001);
